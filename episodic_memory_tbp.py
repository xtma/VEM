...
class EpisodicMemoryTBP(EpisodicMemory):
    ...
    def __init__(self,
                 ...
                 noise_clip=None,
                 max_action=None,):
        super(EpisodicMemoryTBP, self).__init__(
            buffer_size,
            state_dim,
            action_shape,
            obs_space,
    ...
    def update_memory(self, q_base=0, use_knn=False, beta=-1):
        ...
        for traj in trajs:
            ...
            if beta > 0:
                double_rtn = [[
                    np.dot(rtn_2[i, :min(i + 1, self.max_step)], discount_beta[:min(i + 1, self.max_step)]) /
                    np.sum(discount_beta[:min(i + 1, self.max_step)]),
                    np.dot(rtn_1[i, :min(i + 1, self.max_step)], discount_beta[:min(i + 1, self.max_step)]) /
                    np.sum(discount_beta[:min(i + 1, self.max_step)])
