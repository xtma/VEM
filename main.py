...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy", default="VEM")  # Policy name (TD3, DDPG or OurDDPG)
    ...
    reward_l = []
    for seq in dataset:
        observations, actions, dones, rewards, truly_dones = seq['observations'], seq['actions'], seq['timeouts'], seq[
            'rewards'], seq['terminals']
        if dones[-1]:
            truly_dones[-1] = True
        elif truly_dones[-1]:
        ...
        trajectory = [
            (obs, action, 0, 0, reward, truly_done, done)
            for obs, action, reward, truly_done, done in zip(observations, actions, rewards, truly_dones, dones)
        ]
    ...
    for t in range(int(args.max_timesteps)):
        if t % train_freq == 0:
            ...
            print('update_t: ', t, 'time: ', int((time.time() - start_time)), str(args.env), str(slope), str(max_step),
                  str(policy_beta), str(args.seed))
            start_time = time.time()
    ...
    for t in range(int(args.max_timesteps)):
        if t % train_freq == 0:
            for grad_step in range(gradient_steps):
                return_info = policy.train_actor(memory, args.batch_size, gradient_steps, slope, policy_beta,
                                                 threshold_vem, args.env)
        if (t + 1) % args.eval_freq == 0:
