...
class VEM(object):
    ...
    def __init__(self,
        ...
        self.actor_stc = Actor(obs_dim, act_dim, (256, 256, 256, 256), nn.ReLU, max_action).to(device)
    ...
    def train_critic(self, memory, batch_size=128, gradient_steps=100, slope=0.1, policy_beta=0.5):
        ...
        num_samples_collection = {"identical": batch_size, "inner": batch_size * num_q // 2, "both": batch_size * num_q}
        num_samples = num_samples_collection[double_type]
        batch = memory.sample(num_samples, mix=False)
        batch_obs, batch_actions, batch_rewards, batch_next_obs, batch_dones, batch_returns = torch.FloatTensor(
            batch['obs0']).to(device), torch.FloatTensor(batch['actions']).to(device), torch.FloatTensor(
                batch['rewards']).to(device), torch.FloatTensor(batch['obs1']).to(device), torch.FloatTensor(
                    batch['terminals1']).to(device), batch['return']
        ...
        for param, target_param in zip(self.critic.parameters(), self.critic_target.parameters()):
            target_param.data.copy_((1 - (1 - self.tau)**(gradient_steps * 1)) * param.data +
                                    (1 - self.tau)**(gradient_steps * 1) * target_param.data)
